...
library Address {
     ...
     * Requirements:
     *
     * - the calling contract must have an XDAI balance of at least `value`.
...
interface IHoneyswapV1Router01 {
...
interface IHoneyswapV1Router02 is IHoneyswapV1Router01 {
...
contract Nozomics is Context, IERC20, Ownable {
    ...
    IHoneyswapV1Router02 public immutable HoneyswapV1Router;
    address public immutable HoneyswapV1Pair;
    ...
    constructor (address tokenOwner,string memory name, string memory symbol,uint8 decimal, uint256 amountOfTokenWei,uint8 setTaxFee, uint8 setLiqFee, uint256 _maxTaxFee, uint256 _maxLiqFee, uint256 _minMxTxPer) public {
        ...
        IHoneyswapV1Router02 _HoneyswapV1Router = IHoneyswapV1Router02(router);
         // Create a Honeyswap pair for this new token
        HoneyswapV1Pair = IHoneyswapV1Factory(_HoneyswapV1Router.factory())
            .createPair(address(this), _HoneyswapV1Router.WXDAI());

        // set the rest of the contract variables
        HoneyswapV1Router = _HoneyswapV1Router;
    ...
    function setSwapAndLiquifyEnabled(bool _enabled) public onlyOwner {
     ...
     //to receive XDAI from HoneyswapV1Router when swapping
    ...
    function _transfer(
        ...
        // also, don't swap & liquify if sender is Honeyswap pair.
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // capture the contract's current XDAI balance.
        // this is so that we can capture exactly the amount of XDAI that the
        // swap creates, and not make the liquidity event include any XDAI that
        // has been manually sent to the contract
        uint256 initialBalance = address(this).balance;

        // swap tokens for XDAI
        swapTokensForXDAI(half); // <- this breaks the XDAI -> HATE swap when swap+liquify is triggered

        // how much XDAI did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);

        // add liquidity to Honeyswap
        addLiquidity(otherHalf, newBalance);
        
        emit SwapAndLiquify(half, newBalance, otherHalf);
    }

    function swapTokensForXDAI(uint256 tokenAmount) private {
        // generate the Honeyswap pair path of token -> wXDAI
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = HoneyswapV1Router.WXDAI();

        _approve(address(this), address(HoneyswapV1Router), tokenAmount);

        // make the swap
        HoneyswapV1Router.swapExactTokensForXDAISupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of XDAI
            path,
            address(this),
            block.timestamp
        );
    ...
    function addLiquidity(uint256 tokenAmount, uint256 XDAIAmount) private {
        // approve token transfer to cover all possible scenarios
        _approve(address(this), address(HoneyswapV1Router), tokenAmount);

        // add the liquidity
        HoneyswapV1Router.addLiquidityXDAI{value: XDAIAmount}(
            address(this),
            tokenAmount,
